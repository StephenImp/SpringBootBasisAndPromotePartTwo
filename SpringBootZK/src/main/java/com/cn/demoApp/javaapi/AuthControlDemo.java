package com.cn.demoApp.javaapi;
...
public class AuthControlDemo implements Watcher{
    ...
    public static void main(String[] args) throws IOException, InterruptedException, KeeperException {
        ...
        ACL acl=new ACL(ZooDefs.Perms.CREATE, new Id("digest","root:root"));//username:password
        ACL acl2=new ACL(ZooDefs.Perms.CREATE, new Id("ip","192.168.1.1"));

        List<ACL> acls=new ArrayList<>();
        acls.add(acl);
        acls.add(acl2);
        zookeeper.create("/auth1","123".getBytes(),acls,CreateMode.PERSISTENT);//持久化节点
